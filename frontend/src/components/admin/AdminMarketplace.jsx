...
const AdminMarketplace = () => {
  ...
  const fetchTips = async () => {
    try {
      const response = await api.get(`/api/marketplace/tips`, {
        params: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Remove endTime for free and premium tips
      if (tipType !== 'premiumcode') {
        delete data.endTime;
      }
  ...
  const handleStatusUpdate = async (tipId, status) => {
    try {
      await api.patch(`/api/marketplace/tips/${tipId}/status`, {
        status,
        type: activeTab.replace('Tips', '').toLowerCase()
      });
      fetchTips();
